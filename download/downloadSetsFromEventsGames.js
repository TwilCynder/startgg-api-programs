...
import { getEventsSetsGames, getEventsSetsGamesFromObjects, getEventsSetsGamesHashmap, getEventsSetsGamesSeparated } from "../include/getEventsSetsGames.js";
...
if (mode.startsWith("o")){
    ...
    data = await getEventsSetsGamesFromObjects(client, events, limiter, progressManager);
} else {
    events = events.concat(eventObjects.map(event => event.slug).filter(v=>!!v));
    if (mode.startsWith("a")){
        data = await getEventsSetsGamesSeparated(client, events, limiter, progressManager);
    } else if (mode.startsWith("f")){
        data = await getEventsSetsGames(client, events, limiter, progressManager);
    } else {
        data = await getEventsSetsGamesHashmap(client, events, limiter, progressManager);
    }
}
limiter.stop();
