...
import { getSetsCharsInEvents, getSetsCharsInEventsFromObjects, getSetsCharsInEventsHashMap, getSetsCharsInEventsSeparated } from "../include/getCharactersInEvent.js"
...
if (mode.startsWith("o")){
    ...
    data = await getSetsCharsInEventsFromObjects(client, eventObjects, limiter, progressManager);
} else {
    events = events.concat(eventObjects.map(event => event.slug).filter(v=>!!v));
    if (mode.startsWith("a")){
        data = await getSetsCharsInEventsSeparated(client, events, limiter, progressManager);
    } else if (mode.startsWith("f")){
        data = await getSetsCharsInEvents(client, events, limiter, progressManager);
    } else {
        data = await getSetsCharsInEventsHashMap(client, events, limiter, progressManager);
    }
}
limiter.stop();
