...
async function getEventsPage(client, slug, limiter = null, page, config){
    let data = await query.execute(client, {slug, eventsPage: page, games: config.games, minEntrants: config.minEntrants, eventsPerPage: 100}, limiter, false);
...
async function getEventsFromUser_(client, slug, limiter, config){
    if (!config.startDate && !config.endDate){ //we don't have to check each page, we can go for a simple paginated query
        return query.executePaginated(client, {slug, games: config.games, minEntrants: config.minEntrants}, "user.events", limiter, {pageParamName: "eventsPage", perPage: 100});
    } else {
        let result = [];
...
* @param {TimedQuerySemaphore} limiter 
* @param {import('./userEventsQueriesUtil.js').RawUserEventsConfig} config 
* @param {number} minimumIn 
* @returns 
*/
export async function getEventsFromUsers(client, slugs, limiter, config, minimumIn){
    ...
    if (minimumIn){
        for (let list of results){
            ...
            for (let event of list){
                if (!dict[event.id]){
                    dict[event.id] = {event, count: 1}
                } else {
                    dict[event.id].count++;
                }
            }
        }
        list = Object.values(dict).filter(obj => obj.count >= minimumIn).map(obj => obj.event);
    } else {
        for (let list of results){
            if (!list) continue;
